                   ...
                   int index)
{
    ...
    while (j < len)
    {
        ...
        if (j > 0)
        {
            ...
            // compute overlaping
            overlap = fabs(delta) < dupMaxPriDelta * ch->fontSize && fabs(ch->yMin - chPrev->yMin) <
...
void TextPage::updateFont(GfxState *state)
{
    ...
    if (gfxFont && gfxFont->getType() == fontType3)
    {
        ...
        for (code = 0; code < 256; ++code)
        {
            ...
            if (letterCode < 0 && name && name[1] == '\0' && ((name[0] >= 'A' && name[0] <= 'Z') || (name[0] >= 'a' && name[0] <= 'z')))
            {
                letterCode = code;
            }
                                ...
                                GBool isNonUnicodeGlyph)
{
    ...
    if (curWord && curWord->len > 0)
    {
        ...
        overlap = fabs(delta) < dupMaxPriDelta * curWord->fontSize && fabs(base - curWord->base) <
    ...
    if (uLen != 0)
    {
        ...
        if ((curWord->rot == 0 && w1 < 0) || (curWord->rot == 3 && h1 < 0) || (curWord->rot == 2 && w1 > 0) || (curWord->rot == 1 && h1 > 0))
        {
            endWord();
...
{
    if (((fontName && fontName->cmp(tsi->getFontName()) == 0) && (fontSize == tsi->getFontSize()) && (fontColor && fontColor->cmp(tsi->getFontColor()) == 0) && (fontType == tsi->getFontType()) && (fontWidth == tsi->getFontWidth()) && (isbold == tsi->isBold()) && (isitalic == tsi->isItalic()) && (issubscript == tsi->isSubscript()) && (issuperscript == tsi->isSuperscript())))
        return gTrue;
    else
        return gFalse;
}
...
void TextPage::dump(GBool noLineNumbers, GBool fullFontName, vector<bool> lineNumberStatus)
{
    ...
    for (wordId = 0; wordId < words->getLength(); wordId++)
    {
        ...
        if (nextWord != NULL && (word->rot == nextWord->rot) && (((fabs(word->base - nextWord->baseYmin) < maxSpacingWordsBetweenTwoLines) ||
                                                                 ...
                                                                 (nextWord->xMin <= word->xMax + maxWordSpacing * lineFontSize)))
        {
            ...
            if (
                ((rotation == -1) ? ((word->base < nextWord->yMin) && (word->yMin < nextWord->yMin)) : (word->rot == 0 || word->rot == 1) ? ((word->base < yyMinNext) && (yyMin < yyMinNext))
                                                                                                                                          : ((word->base > yyMinNext) && (yyMin > yyMinNext))) ||
                ((rotation == -1) ? (nextWord->xMin < word->xMin) : (word->rot == 0) ? (xxMinNext < xxMin)
                                                                                     : (word->rot == 1 ? xxMinNext > xxMin : (word->rot == 2 ? xxMinNext > xxMin : xxMinNext < xxMin))) ||
                ((rotation == -1)                     ? (nextWord->xMin < word->xMax) && (fabs(nextWord->base - word->base) > maxIntraLineDelta * lineFontSize)
                 : (word->rot == 0 || word->rot == 3) ? ((xxMinNext < xxMax) && (fabs(nextWord->base - word->base) > maxIntraLineDelta * lineFontSize))
                                                      : ((xxMinNext > xxMax) && (fabs(nextWord->base - word->base) > maxIntraLineDelta * lineFontSize))) ||
                ((rotation == -1) ? (nextWord->xMin > word->xMax + maxWordSpacing * lineFontSize) : (word->rot == 0 || word->rot == 3) ? (xxMinNext > xxMax + maxWordSpacing * lineFontSize)
        ...
        if ((lineFinish) || (endPage))
        {
            ...
            if (linePreviousX == 0)
            {
                if (nextWord != NULL)
                {
                    if (nextWord->xMin > (lineX + lineWidth) + (maxColSpacing * lineFontSize))
                    {
                        newBlock = gTrue;
                    }
                }
            ...
            else
            {
                ...
                {
                    if (((lineYmin + lineHeight) >= linePreviousYmin) && (fabs(lineFontSize - linePreviousFontSize) < lineFontSize * maxBlockFontSizeDelta1) && ((lineYmin - linePreviousYmin) < (linePreviousFontSize * maxLineSpacingDelta)))
                    {
                        if (endPage)
                        {
    ...
    for (parIdx = 0; parIdx < blocks->getLength(); parIdx++)
    {
        ...
        for (lineIdx = 0; lineIdx < par->lines->getLength(); lineIdx++)
        {
            ...
            if (nonEmptyLine){
                ...
                printf("Not adding line to block :\n");
            }
        }
                                ...
                                Unicode *u, int uLen)
{
    ...
    if (parameters->getOcr() == gTrue)
    {
             ...
             (uLen > 1 && (globalParams->getTextEncodingName()->cmp(ENCODING_UTF8) == 0) && !isUTF8(u, uLen))))
        {
            ...
            if (state->getFont()->getName())
            { // AA : Here if fontName is NULL is problematic
...
void XmlAltoOutputDev::stroke(GfxState *state)
{
    ...
    if (length != 0)
    {
        ...
        for (i = 0; i < length; i++)
        {
            snprintf(tmp, sizeof(tmp), ATTR_NUMFORMAT, state->transformWidth(dash[i]) == 0 ? 1 : state->transformWidth(dash[i]));
            attr->append(tmp);
    ...
    // if (parameters->getDisplayImage())
    {
        ...
        if ((ref != NULL) && (ref->isRef()))
        {
            ...
            for (int i = 0; i < lPictureReferences->getLength(); i++)
            {
                ...
                if ((pic_reference->reference_number == reference) && (pic_reference->picture_flip == flip))
                {
